...
flock -w 120 ${analysis_dir_base}_${era}/${sample}_${channel}_command.log echo Job ${job_number} python FTAnalyzer.py ${stage} --variableSet ${var_set} --categorySet ${cat_set} --analysisDirectory ${analysis_dir_base}_${era} --noAggregate --channel ${channel} --bTagger ${btagger} --jetPUId ${jetpuid} --include ${sample} --nThreads ${num_threads} --source NANOv7_CorrNov__${channel} --sample_cards '../Kai/python/samplecards/'${era}'_NanoAODv7.yaml' '../Kai/python/samplecards/'${era}'_NanoAODv7_additional.yaml' --systematics_cards '../Kai/python/samplecards/'${era}'_systematics_NanoV7_V8.yaml' --noProgressBar --era ${era} >> ${analysis_dir_base}_${era}/${sample}_${channel}_command.log

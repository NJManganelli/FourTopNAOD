            ...
            for processName in nodes:
                ...
                for decayChannel in ["ElMu{lpf}".format(lpf=leppostfix), 
                    ...
                    if decayChannel == "ElMu{lpf}".format(lpf=leppostfix):
                        channelFilter = "nFTALepton{lpf} == 2 && nFTAMuon{lpf} == 1 && nFTAElectron{lpf}== 1 && FTALepton{lpf}_pt.at(1) >= 25".format(lpf=leppostfix)
                        ...
                        L0String = "HT{bpf} >= {htc} && {met} >= {metcut} && FTAMuon{lpf}_InvariantMass > 20 && abs(FTAMuon{lpf}_InvariantMass - 91.2) > {zwidth} && FTALepton{lpf}_pt.at(1) >= 25"\
                        ...
                        L0String = "HT{bpf} >= {htc} && {met} >= {metcut} && FTAElectron{lpf}_InvariantMass > 20 && abs(FTAElectron{lpf}_InvariantMass - 91.2) > {zwidth} && FTALepton{lpf}_pt.at(1) >= 25"\
                            .format(lpf=leppostfix, met=fillMET_pt, metcut=ZMassMETWindow[1], zwidth=ZMassMETWindow[0], bpf=branchpostfix, htc=HTCut)
    ...
    for name, vals in theSampleDict.items():
        ...
        for lvl in levelsOfInterest:
            ...
            print("Took {}m {}s ({}s) to process {} events from sample {} in channel {}\n\n\n{}".format(theTime//60, theTime%60, theTime, processed[name][lvl], 
                         name, lvl, "".join(["\_/"]*25)))
    print("No benchmark here yet")# Benchmark.Summary()
    return packedNodes
def otherFuncs():

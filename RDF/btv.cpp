...
ROOT::RDF::RNode apply_btv_sfs(ROOT::RDF::RNode df, 
  ...
  if(wp == "shape_corr"){
    ...
    if(algo == "deepcsv")
      auto cMap = cSet->at("deepCSV_shape_" + era);
    else if(algo == "deepjet")
	...
	for(int i = 0; i < hadronFlavour.size(); ++i){
	  ...
	  inputs.emplace_back(std::clamp(abs(static_cast<double>(eta.at(i))), 0., 2.5-0.000001));
	  inputs.emplace_back(std::clamp(static_cast<double>(pt.at(i)), 20.0001, 9999.999));

...
def main(stage, analysisDirectory, channel, era, relUncertainty, jsonInput, outDir, sourceVariable = "HT", HTCut="500", 
         variableSet="HTOnly", categorySet="5x5", merge="", tagger="DeepJet", systematicsToExclude=[], nEquiprobableBins=None, includeOverflow=False, verbose=False):
    if nEquiprobableBins:
        print("\n--nEquiprobableBins {} overrides relUncertainty\n".format(nEquiprobableBins))
    ...
    if verbose:
        ...
        if nEquiprobableBins:
            print("Systematics: {} [for nEquiprobableBins, reduced to one postfix]".format(systematics))
        else:
        ...
        for channelWindow in channelWindows:
            ...
            for category in categories:
                ...
                for variable in variables:
                    ...
                    edgesArray = np.zeros(nBinsX+3, dtype=np.float32)
                    # binningArray.append(0) #Can't rebin the overflow anyway... thanks ROOT
                    for nsyst, systematic in enumerate(systematics):
                        lastBin = hists[systematic].GetNbinsX()+2
                        for nbin in range(0, lastBin): #Include underflow and overflow for bin numbering clarity                            
...
if __name__ == '__main__':
    ...
    parser.add_argument('--OF', '--includeOverflow', dest='includeOverflow', action='store_true',
                        help='Include overflow when determining the binning')

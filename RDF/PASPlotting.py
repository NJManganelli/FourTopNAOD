    ...
    for can_name, can_dict in sorted(canvases.items(), key=lambda x: x[0].split("_")[-1], reverse=False):
        ...
        for pn, subplot_name in enumerate(CanCache["subplots"]):
            ...
            for categoryorsupercategory in CanCache["subplots/supercategories"][pn]['Supercategories/hists']:
                ...
                handle['Supercategories/statSystematicErrors'][supercategory].SetFillStyle(3344)
                ...
                handle['Supercategories/statSystematicErrors/ratio'][supercategory].SetFillStyle(3344)
                ...
                if pn == 0:
                    if ratioUncertainties or histogramUncertainties:
                        if "Uncertainty" not in [x.GetLabel() for x in CanCache["subplots/supercategories"][0]["Legend"].GetListOfPrimitives()]:
                            CanCache["subplots/supercategories"][0]["Legend"].AddEntry(handle['Supercategories/statSystematicErrors'][supercategory], "Uncertainty", "F")
                            CanCache["subplots/supercategories"][0]["Legend2"].AddEntry(handle['Supercategories/statSystematicErrors'][supercategory], "Uncertainty", "F")
            ...
            if doRatio:
                ...
                for aRatioName, aRatio in legendConfig.get("Ratios", defaults["DefaultLegend"].get("Ratios")).items():
                    ...
                    if ratioUncertainties:
                        ...
                        #         scGraph.Draw("5") #2 does fill rectangle, 5 adds the line around it
                        #         redraw = True

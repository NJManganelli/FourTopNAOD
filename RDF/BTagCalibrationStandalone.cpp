...
double BTagCalibrationReader::BTagCalibrationReaderImpl::eval_auto_bounds(
                                             ...
                                             float discr) const
{
//   auto jf = BTagEntry::jetFlavourFromHadronFlavour(hf);
// class BTagEntry
//   enum JetFlavor {
//     FLAV_B=0,
//     FLAV_C=1,
//     FLAV_UDSG=2,
//   };
  BTagEntry::JetFlavor jf = [](int d){auto t = abs(d); 
				      if(t == 5) return BTagEntry::FLAV_B; 
				      else if(t == 4) return BTagEntry::FLAV_C; 
				      else if((t < 4) || (t == 21)) return BTagEntry::FLAV_UDSG; 
				      else throw std::invalid_argument("received invalid Jet_hadronFlavour");
  }(hf);
  auto sf_bounds_eta = min_max_eta(jf, discr);
  bool eta_is_out_of_bounds = false;
...
double BTagCalibrationReader::eval_auto_bounds(const std::string & sys,
                                               ...
                                               float discr) const
{
  return pimpl->eval_auto_bounds(sys, hf, eta, pt, discr);
}

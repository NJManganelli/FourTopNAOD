...
def main(input1, input2, Ch22Maximum, KSMinimum, sort_key, maxResults, verbose=False):
    ...
    print("Histogram keys...\n\tOnly in input1: {}\n\tOnly in input2: {}\n\tIn common: {}".format(len(kOnlyInOne), len(kOnlyInTwo), len(kInBoth)))
    ...
    Results = [(kk, KSTestResults[kk], Chi2TestResults[kk]) for kk in kInBoth]
    sort_function = None
    if sort_key == 'Chi2':
    ...
    for result in Results[:maxToPrint]:
        print("KS: ", result[1], " Chi2/NDoF: ", result[2], " Histogram: ", result[0])
# doCSV = False
# if doCSV:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sort', dest='sort', action='store', choices=['KS', 'Chi2', 'Name', 'name'], default='Chi2', type=str,
                        help='Sorting key, defaulting to descending Chi2 test values; KS sorts in ascending order')
    ...
    main(args.input1, args.input2, args.Chi2Maximum, args.KSMinimum, args.sort, args.maxResults, verbose=verbose)
